...
export function encryptQueryParamWithHMAC(text: string): string {
  ...
  const cipher = crypto.createCipheriv("aes-256-gcm", Buffer.from(SECRET_KEY, "utf-8"), iv);
  let encrypted = cipher.update(text, "utf8", "base64");
  encrypted += cipher.final("base64");
  const authTag = cipher.getAuthTag();

  // Combine IV + encrypted data + authTag
  const combined = Buffer.concat([iv, Buffer.from(encrypted, "base64"), authTag]).toString("base64url");
 ...
 */
export function decryptQueryParamWithHMAC(encryptedText: string): string | null {
  ...
  const encryptedContent = encryptedBuffer.subarray(IV_LENGTH, encryptedBuffer.length - 16);

  const decipher = crypto.createDecipheriv("aes-256-gcm", Buffer.from(SECRET_KEY, "utf-8"), iv);
