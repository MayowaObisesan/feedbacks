...
export default function FeedbacksForm({width, height, isPreview=true }: {width?: number; height?: number; isPreview: boolean}) {
  ...
  const onCreateFeedback = async () => {
    ...
    try { // get the brandId from the API-Key provided.
      if (thisBrandData) {
        ...
        if (data) {
          ...
          const { result: verifyResult, error: verifyError } = await verifyKey({ key: apiKey, apiId: result?.apiId });
          if (verifyError) {
      ...
      toast.error("Error submitting feedback", {description: "An error occurred trying to submit feedback. Pls try again."})
    } finally {
      setIsSubmitting(false)
    }
        ...
        <Card className="max-w-[400px]">
          ...
          <CardBody>
            <div>
              ...
              <br/> <br/>
              Please drop a feedback for the brand from our official Website using the link below.
              <br/> <br/>
              Just search for this brand name and drop them a feedback.
  ...
  return (
    <ScrollShadow>
      <div className={"p-4 space-y-8"} style={{ width: `${queryWidth}px`, height: `${queryHeight}px` }}>
        {/*<User
          avatarProps={{
        ...
        <div className={"flex flex-row justify-between items-center gap-x-2"}>
          <div className={""}>
            Send Feedback to
            <div className={"font-bold text-2xl"}>{thisBrandData?.rawName}</div>
          </div>
          <Dropdown placement="bottom-start" isDisabled={isPreview}>
            <DropdownTrigger>
              <User
                as="button"
                avatarProps={{
            ...
            <DropdownMenu aria-label="User Actions" variant="flat" classNames={{
              list: "gap-y-2"
            }}>
              <DropdownItem key="profile" className="h-14 gap-2">
                <p className="font-medium text-xs text-default-400">Signed in as</p>
                <div>
                  <div className="font-bold">{user?.user_metadata.full_name}</div>
                  <div className="text-sm text-default-500 overflow-ellipsis overflow-x-hidden whitespace-nowrap">({userDB?.email})</div>
                </div>
              </DropdownItem>
              <DropdownItem key="system">
                <Link isExternal showAnchorIcon href={FEEDBACKS_URL} className={"text-sm"}>
                  Visit Feedbacks app
                </Link>
              </DropdownItem>
